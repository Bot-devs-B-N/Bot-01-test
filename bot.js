...
var botConfigs = {
    ...
    welcomemessage: {
    "channelid": "375448423169523712",
    "text": "welcome, We have been expecting you."
},
    weather: {
    "degree": "C"
},
    autorole: {
    "guildID": "672195129179766813",
    "autoroleID": "672224185363202079"
},
    ticketsystem: {
    "ticketCategoryID": "683674158504083464",
    "createTicketChannelID": "663147882550919198"
}
};
...
let roles = ["686003685859524620", "686251320860213259", "686003954638782515", "686004091272298542", "686004091272298542"];
let emojis = ["655626561524989982", "661037275496316954", "651169651518341130", "651200950916349965", "650872466922536987"];

client.on('messageReactionAdd', async (reaction, user) => {

  if(reaction.message.channel.id !== "686008424093188337") return;
  for(let i in emojis) {
    if(reaction.emoji.id ==  emojis[i]) {
      let role = reaction.message.guild.roles.find(f => f.id === roles[i])
...
client.on('messageReactionRemove', async (r, user) => {
    ...
    if(r.message.channel.id !== "686008424093188337") return;
    for(let i in emojis) {
      if(r.emoji.id ==  emojis[i]) {
        let role = r.message.guild.roles.find(f => f.id === roles[i])
        let member = r.message.guild.members.get(user.id)
        if(member.roles.has(role.id)) {
          member.removeRole(role)
        } else {
          return;
...
client.on("guildMemberAdd", async function(member) {
    ...
    if(member.guild.id == "318087636981907457") {
        member.send("Hello, Welcome to the Storm vipers discord server. If you would like to join the clan go to <#686008424093188337> and pick which clan you would like to join. Enjoy your stay!")
    }
...
client.on("message", async function (message) {
  ...
  if(command === 'setup') {
    if(message.author.id !== '364095679271927809') return;
    await message.delete()
    
    const setUp = new Discord.RichEmbed()
      .setDescription('React to reward roles :tada:')
      .addField('🎮 Roles',
                  message.guild.emojis.get("655626561524989982") + ' : **`Storm Vipers`**\n\n'
                  + message.guild.emojis.get('661037275496316954') + ' : **`Storm Vipers EU`**\n\n'
                  + message.guild.emojis.get('651169651518341130') + ' : **`Storm Vipers X`**\n\n'
                  + message.guild.emojis.get('651200950916349965') + ' : **`Storm Vipers II`**\n\n'
                  + message.guild.emojis.get('650872466922536987') + ' : **`Storm Vipers Elite`**\n\n'
               )
      .setColor(0x36393f)
      // .setFooter('Development by: PiRo#8300')
    
    message.channel.send(setUp).then(async m => {
      await m.react("655626561524989982");
      await m.react("661037275496316954");
      await m.react("651169651518341130");
      await m.react("651200950916349965");
      m.react("650872466922536987");
    }) 
  }
    ...
    if (command === "lockdown" && botConfigs.plugins[9].activated == true) {
        ...
        let roleID = "334077926821593100";
        if (validUnlocks.includes(time)) {
            message.channel.overwritePermissions(roleID, { SEND_MESSAGES: null }).then(() => {
                message.channel.send({ embed: Unlockembed });
                clearTimeout(client.lockit[message.channel.id]);
                delete client.lockit[message.channel.id];
            }).catch(error => { console.log(error); });
        } else {
            message.channel.overwritePermissions(roleID, { SEND_MESSAGES: false }).then(() => {
                message.channel.send({ embed: Lockembed }).then(() => {
                    client.lockit[message.channel.id] = setTimeout(() => {
                        message.channel.overwritePermissions(roleID, {
                            SEND_MESSAGES: null
